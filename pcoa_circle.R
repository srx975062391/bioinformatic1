...
#
# 输入文件为距离矩阵(如unweighted_unifrac.txt)+分组信息(metadata.tsv)
#
# 输入文件"-i", "--input"，beta/unweighted_unifrac.txt; beta多样性距离矩阵文件，有多种距离可选，常用bray_curtis、jaccard、unifrac和unifrac_binary
#
...
if (TRUE){
  option_list = list(
    make_option(c("-i", "--input"), type="character", default="beta/unweighted_unifrac.txt",
...
# 设置输出文件缺省值，如果为空，则为输入+pcoa_circle.pdf
#if(opts$output==""){opts$output=paste0(opts$input,".pcoa_circle.pdf")}
...
s.class(pcoa$li,A,cpoint = 1.2,col=grp_cols,cellipse =1,axesell = F,addaxes = T,add.plot=TRUE,clabel=0)
